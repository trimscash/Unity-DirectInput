...
HRESULT UpdateFFBEffectWithDirection(LPCSTR guidInstance, Effects::Type effectType, const std::vector<DICONDITION>& conditions, const std::vector<LONG>& direction) {
  ...
  hr = _DeviceFFBEffectControl[GUIDString][effectType]->SetParameters(&_DeviceFFBEffectConfig[GUIDString][effectType], DIEP_TYPESPECIFICPARAMS | DIEP_DIRECTION);
  return hr;
}
...
HRESULT UpdateFFBEffectDirection(LPCSTR guidInstance, Effects::Type effectType, const std::vector<LONG>& direction) {
  ...
  hr = _DeviceFFBEffectControl[GUIDString][effectType]->SetParameters(&_DeviceFFBEffectConfig[GUIDString][effectType], DIEP_DIRECTION);
  return hr;
}
